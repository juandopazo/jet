...
else if(c.length&gt;=2&amp;&amp;c.charAt(0)=="?")c=c.substring(1);c=c!=a}c=c}if(c)b.src=e.get("blankURL")+a;else if(f.ie)return;k=a;t=false;A(a)}}},J=function(){d(d.win).on("hashchange",function(){A(d.win.location.hash)})};x.iframe.on("load",function(a,c){if(h)h=false;else{var i=String(c.search);if(i.charAt(0)=="?")i=i.length==1?"":i.substring(1);window.location.hash=i;A(i)}});this.initialize=function(){m=document.title;if(f.ie)if(q.hasKey("DhtmlHistory_pageLoaded")){s=true;j=false}else{s=false;j=true;q.put("DhtmlHistory_pageLoaded",
true)}return e};this.changeTitle=function(a){a=a&amp;&amp;a.newTitle?e.get("baseTitle").replace("@@@",a.newTitle):m;if(document.title==a)return e;document.title=a;if(f.ie)b.contentWindow.document.title=a;if(!f.ie&amp;&amp;!f.opera){a=g(document.location.hash);if(a!==""){a=l(z(a));document.location.hash=a}}return e};this.go=function(a,c){var i=[];if(w.isHash(a)){F.each(a,function(u,K){i.push(u+"="+K)});a=i.join("&amp;")}var o=l(z(a));window.setTimeout(function(){if(n&gt;0)n-=p;if(document.getElementById(o)&amp;&amp;e.get("debugMode")){var u=
"Exception: History locations can not have the same value as _any_ IDs that might be in the document,";u+=" due to a bug in IE; please ask the developer to choose a history location that does not match any HTML";u+=" IDs in this document. The following ID is already taken and cannot be a location: "+a;throw new Error(u);}q.put(a,c);t=h=true;k=o;window.location.hash=o;if(f.ie)b.src=e.get("blankURL")+o;t=false;e.changeTitle(c)},n);n+=p;return e};q.setup({debugMode:e.get("debugMode")});var D=C();k=D;
if("onhashchange"in d.win)J();else{if(f.opera){p=400;d("&lt;img/&gt;").style(v).attr("src","javascript:location.href='javascript:dhtmlHistory.checkLocation();';").appendTo(B)}f.ie&amp;&amp;H(D);d(d.win).on("unload",function(){j=null});if(f.ie)h=true;else if(q.hasKey("DhtmlHistory_pageLoaded")){j=h=false;s=true}else{j=h=true;q.put("DhtmlHistory_pageLoaded",true)}setInterval(I,100)}};d.extend(y,d.Base);d.History=y});
