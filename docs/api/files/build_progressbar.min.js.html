...
*/
jet.add("progressbar",function(d){d.ProgressBar=d.Base.create("progressbar",d.Widget,[],{ATTRS:{minValue:{value:0},maxValue:{value:100},value:{value:0},width:{value:200},height:{value:20},direction:{value:"ltr",writeOnce:true},animate:{value:false,writeOnce:true,validator:function(){return!!d.Tween}},easing:{},duration:{value:500},bar:{readOnly:true,value:d("&lt;span/&gt;")}},EVENTS:{valueChange:function(a){this.fire("progress",{value:a.newVal})?this._update(a.newVal):a.preventDefault()},directionChange:function(a,
c,b){this.get("boundingBox").removeClass(this.getClassName(b)).addClass(this.getClassName(c))},render:function(){var a=this.get("direction"),c=this.get("bar").appendTo(this.get("contentBox")).addClass(this.getClassName("bar"));this.get("boundingBox").addClass(this.getClassName(a));a=="ltr"?c.height(this.get("height")):c.width(this.get("width"))}}},{_onTween:function(a){this.fire("progress",{value:a.value})||a.preventDefault()},_onTweenEnd:function(){this.fire("end")},_update:function(a){var c=this.get("minValue");
a=(a-c)/(this.get("maxValue")-c);var b=this.get("bar");c=this.get("width");var e=this.get("height"),f=this.get("direction"),g=this.get("easing"),h=this.get("duration");if(this.get("animate")){if(this._tween){this._tween.unbind("tween",this._onTween);this._tween.unbind("end",this._onTweenEnd);this._tween.destroy()}b=this._tween=new d.Tween({node:b,duration:h,easing:g});b.on("tween",this._onTween,this);b.on("end",this._onTweenEnd,this);switch(f){case "ttb":case "btt":b.set("to",{height:e*a});break;
default:b.set("to",{width:c*a})}b.play()}else{switch(f){case "ttb":case "btt":a*=e;b.height(a);break;default:a*=c;b.width(a)}this.fire("progress",{value:a});this.fire("end")}}})});
