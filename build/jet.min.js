...
s.flush);var B=C("div",{id:"jet-tracker"},{position:"absolute",width:"1px",height:"1px",top:"-1000px",left:"-1000px",visibility:"hidden"});r.body.appendChild(B);q.jet=function(c){c=c||{};var f=J;if(c.base){f=c.base;f=f.substr(f.length-1,1)=="/"?f:f+"/"}c.minify=m.isBoolean(c.minify)?c.minify:true;var d=u(v(M),c.modules||{},true),e=function(h){G(h.fullpath||f+h.path);var j=setInterval(function(){if((q.getComputedStyle?q.getComputedStyle(B,null):B.currentStyle?B.currentStyle:{})[h.beacon.name]==h.beacon.value){clearInterval(j);
