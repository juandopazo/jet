...
200||b.responseText||b.responseXML?w(s(b,e,j),b):j("Bad status",b.status,b)};try{b.open(i,k,p);b.send(null)}catch(A){j("Bad status",404,b)}if(p===false)c=s(b,e,j);else setTimeout(function(){if(b.readyState!==4){b.abort();j("timeout",408,b)}},h)}}return c||f},jsonp:function(a){a=a||{};var b=a.jsonCallbackParam||"p",d=jet.IO.jsonpCallbacks,c=d.length,e=false,h=a.url;if(h){d[c]=function(i){e=true;a.success&&a.success(i);a.complete&&a.complete(i)};if(h.substr(h.length-1)!="?")h+="?";if(a.data)h+=q(a.data);
f.Get.script(h+b+"=jet.IO.jsonpCallbacks["+c+"]");setTimeout(function(){if(!e){var i={message:"Request failed",reason:"Timeout"};a.error&&a.error(i);a.complete&&a.complete(i)}},a.timeout||1E4)}}});f.IO={utils:{parseXML:r}}});
