 ...
 * https://github.com/juandopazo/jet/blob/master/LICENSE.md
*/
jet.add("node",function(f){function t(a){this._handlers=a||[]}function r(a){return new RegExp("(^|\\s)"+a+"(\\s|$)")}function n(a,b){var c=0,d;b=b||f.context;a=h.isValue(a)?a:[];if(h.isArray(a._nodes))a=a._nodes;else if(h.isString(a))a=[b.createElement(a)];else if(a.nodeType||a.body||a.navigator)a=[a];else if(h.isArray(a))for(;c<a.length;)if(!a[c]||!(a[c].nodeType||a[c].body||a[c].navigator))a.splice(c,1);else c++;else if(h.isNumber(a.length)){d=[];for(c=0;c<a.length;c++)d[c]=a[c];a=d}else f.error("Wrong argument for NodeList");
