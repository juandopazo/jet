...
LINKED_CSS:this.get("linkedcss"),CONTENT:this.get("content"),DEFAULT_CSS:f.DEFAULT_CSS,EXTRA_CSS:d});if(c.config.doc.compatMode!="BackCompat")d=f.getDocType()+"\n"+d;b=this._resolveWinDoc();b.doc.open();b.doc.write(d);b.doc.close();if(b.doc.documentElement)a(b);else var e=setInterval(function(){if(b.doc&&b.doc.documentElement){a(b);clearInterval(e)}},1)},_resolveWinDoc:function(a){a=a?a:{};a.win=this._iframe.getDOMNode().contentWindow;a.doc=this._iframe.getDOMNode().contentWindow.document;if(!a.doc)a.doc=
c.config.doc;if(!a.win)a.win=c.config.win;return a},initializer:function(){},destructor:function(){var a=this.getInstance();a(a.config.doc).unbind();this._iframe.remove(true)},_onContentReady:function(a){if(!this._ready){this._ready=true;this._iframe.css("visibility","");var b=this.getInstance(),d=this.get("use");this.fire("contentReady");if(a)b.config.doc=a.target.getDOMNode()||a.target;b.use.apply(b,d);b(b.config.doc.documentElement).addClass("jet-js-enabled")}},_getHTML:function(a){if(this._ready)a=
...
this.getInstance();b.get("#extra_css").remove();b.one("head").append(c("<style/>").attr("id","extra_css").html(a))}return a},_instanceLoaded:function(a){this._instance=a;this._onContentReady()},getInstance:function(){return this._instance},render:function(a){if(this.get("rendered"))return this;this.set("rendered",true);a&&this.set("container",a);this._create(c.bind(function(b){var d,e,k=c.bind(function(g){g.config.win.jet_Config=c.mix({},c.config);g.config.win.jet=jet;this._instanceLoaded(g)},this),
l=this.get("use"),i={win:b.win,doc:b.doc},m=c.bind(function(){i=this._resolveWinDoc(i);d=jet(i);try{d.use("node",k);e&&clearInterval(e)}catch(g){e=setInterval(function(){m()},350)}},this);l.push(m);c.use.apply(c,l)},this));return this}});h=c.Array;c.WidgetSandbox=c.mix(j,{ATTRS:{extraScripts:{writeOnce:true,value:[]},extraCss:{writeOnce:true,value:[]}},EVENTS:{afterRender:function(){var a=this.frame;a.on("contentReady",this._onFrameReady,this);a.render(this.get("boundingBox"))}}});j.prototype={getInstance:function(){return this.frame.getInstance()},
_onFrameReady:function(){var a=this.frame.getInstance(),b=a.config.doc,d=b.body,e=this.get("contentBox");try{e.appendTo(d)}catch(k){b=b.importNode(e.getDOMNode(),true);d.appendChild(b);this.set("contentBox",b);e.remove()}h.each(this.get("extraScripts"),a.Get.script,a.Get);this.fire("ready")}}});
