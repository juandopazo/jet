...
a)){this._addRow(a);(a=this.get("sortedBy"))&&this._sort(d("#"+this.getClassName(this._uid,"th",a)));this.fire("afterAddRow")}return this},addRows:function(a){if(this.fire("addRows",a)){if(n.isArray(a))q.each(a,function(b){p.hasInstance(b)||new d.Record(b)});else if(v.hasInstance(a))a=a.getRecords();q.each(a,this._addRow,this);(a=this.get("sortedBy"))&&this._sort(d("#"+this.getClassName(this._uid,"th",a)),true);this.fire("afterAddRows")}return this},getColumn:function(a){var b=[],c,f=this.get("recordSet").getRecords(),
h=f.length,e=this.get("columnDefinitions"),g=this.get("tbody").children(),j=n.isNumber(a)?e[a].key:a,l=a;if(!n.isNumber(a))for(c=0;c<e.length;c++)if(e[c].key==a){l=c;break}for(c=0;c<h;c++)b[b.length]=new w({record:f[c],td:g.item(c).children().item(l),value:f[c].get(j)});return new s({cells:b})},getFirstTr:function(){return this.get("tbody").children().item(0)},getNextTr:function(a){if(p.hasInstance(a))a=a.getId();if(n.isNumber(a))a=this.get("tbody").find("#"+this.get("recordIdPrefix")+a);return a.next()},
getFirstTd:function(a){if(p.hasInstance(a))a=a.getId();if(n.isNumber(a))a=this.get("tbody").find("#"+this.get("recordIdPrefix")+a);return a.children().item(0)},getNextTd:function(a){if(p.hasInstance(a))a=a.getId();if(n.isNumber(a))a=this.get("tbody").find("#"+this.get("recordIdPrefix")+a).children(a-1);return a.next()},onDataReturnReplaceRows:function(a){this.get("tbody").children().remove();this.get("recordSet").replace(a.data);this.addRows(a.data)},onDataReturnAddRows:function(a){this.get("recordSet").push(a.data);
this.addRows(a.data)}});d.ScrollingDataTable=o.create("dt",r,[],{ATTRS:{tbodyContainer:{value:"<div/>",setter:d},autoScroll:{value:false},autoScrollStatus:{value:false}}},{initializer:function(){this.set("tbodyContainer",this.get("tbodyContainer"))},_autoScrollBefore:function(){var a=this.get("tbodyContainer");this.set("autoScrollStatus",a._nodes[0].scrollTop+a.height()==a._nodes[0].scrollHeight)},_autoScrollAfter:function(){var a=this.get("tbodyContainer");if(this.get("autoScroll")&&this.get("autoScrollStatus"))a._nodes[0].scrollTop=
