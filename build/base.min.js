...
left:t.left,top:t.top}).hide().appendTo(l.parent())[0])});b=g(s)}if(!i){b.show();f=setInterval(function(){a.fire("mousemove",c,e)},a.get("frequency"));i=true}}else{b.hide();clearInterval(f);i=false}});g(g.context).on("selectstart",function(h){i&&h.preventDefault()});b.link(g(g.context),true).on("mousemove",function(h){c=h.clientX;e=h.clientY;a.get("tracking")&&a.get("shim")&&d.each(function(j,s){j=g(j);var l=j.offset();b[s].height(l.height).width(l.width).css({left:l.left+"px",top:l.top+"px"})})}).on("mouseup",
