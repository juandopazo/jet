...
c){c.html(this.get("text"))}});b.ComboBox=d.create("combobox",g,[h],{ATTRS:{defaultChildType:{value:"ComboOption"},multiple:{value:false,readOnly:true},atLeastOne:{value:true,readOnly:true},displayNode:{value:"<span/>",setter:b},inputNode:{value:"<input/>",setter:b},arrowContainer:{value:"<span/>",setter:b},arrow:{value:"<b/>",setter:b}}},{BOUNDING_TEMPLATE:"<span/>",CONTENT_TEMPLATE:"<ul/>",_uiComboSelectionChange:function(a){if(a.newVal){this.get("displayNode").html(a.newVal.get("text"));this.get("inputNode").attr("value",
a.newVal.get("value"));setTimeout(b.bind(this._setMinWidth,this),0)}},_setMinWidth:function(){var a=this.get("contentBox"),c=this.get("boundingBox").width();a.width()<c&&a.width(c)},_toggleContent:function(a){var c=this.get("contentBox").getDOMNode();b(a.domEvent.target).ancestor(function(e){return e==c}).size()===0&&this.get("boundingBox").toggleClass(this.getClassName("expanded"))},_uiComboHide:function(a){var c=this.get("boundingBox");b(a.target).ancestor(function(e){return e==c.getDOMNode()}).size()===
0&&c.removeClass(this.getClassName("expanded"))},_uiContentHide:function(){this.get("boundingBox").removeClass(this.getClassName("expanded"))},initializer:function(){this.set("displayNode",this.get("displayNode"));this.set("inputNode",this.get("inputNode"));this.set("arrowContainer",this.get("arrowContainer"));this.set("arrow",this.get("arrow"));this.after("addChild",this._setMinWidth);this.on("afterRender",this._setMinWidth)},renderUI:function(a,c){this.get("displayNode").addClass(this.getClassName("display")).prependTo(a);
var e=this.get("arrowContainer").addClass(this.getClassName("arrow","container")).prependTo(a);this.get("arrow").addClass(this.getClassName("arrow")).appendTo(e);this.get("inputNode").attr({type:"hidden",id:this.getClassName("input",this._uid)}).appendTo(a);c.addClass(this.getClassName("collapsed")).on("click",this._uiContentHide,this)},bindUI:function(){this.after("selectionChange",this._uiComboSelectionChange);this.on("click",this._toggleContent);this._handlers.push(b(b.config.doc).on("click",this._uiComboHide,
this))},syncUI:function(){this.get("displayNode").html(this.get("selection").get("text"));this.get("inputNode").attr("value",this.get("selection").get("value"));this._setMinWidth()}});if(!f.isNumber(jet.Button.radio))jet.Button.radio=0;b.RadioButton=d.create("radio",i,[],{EVENTS:{afterSelectionChange:function(a){this.get("contentBox").getDOMNode().checked=!!a.newVal},render:function(){this.get("contentBox").attr({type:"radio",name:this.get("parent").get("name")})}}},{CONTENT_TEMPLATE:"<input/>"});
b.RadioGroup=d.create("radio-group",g,[h],{ATTRS:{multiple:{value:false,readOnly:true},name:{writeOnce:true},defaultChildType:{value:b.RadioButton}}},{initializer:function(){this.set("name",this.getClassName(++jet.Button.radio))}});if(!f.isNumber(jet.Button.checkbox))jet.Button.checkbox=0;b.CheckBox=d.create("checkbox",i,[],{},{CONTENT_TEMPLATE:"<input/>",_uiCheckBoxSelect:function(a){this.get("contentBox").getDOMNode().checked=!!a.newVal},renderUI:function(){this.get("contentBox").attr({type:"checkbox",
name:this.get("parent").get("name")})},bindUI:function(){this.after("selectedChange",this._uiCheckBoxSelect)}});b.CheckBoxGroup=d.create("checkbox-group",g,[h],{ATTRS:{name:{writeOnce:true},defaultChildType:{value:"CheckBox"}}},{initializer:function(){this.set("name",this.getClassName(+jet.Button.checkbox))}})});
