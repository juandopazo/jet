...
arguments);var a=this.addAttrs({options:{value:[]},buttonNode:{readOnly:true,value:f("<select/>")},className:{value:"combobox"},selected:{validator:m.isNumber,getter:function(){var d=a.get("buttonNode")[0];return d.options[d.selectedIndex]},setter:function(d){var e=a.get("buttonNode")[0];if(d>=0&&a.fire("change",e.options[d]))e.selectedIndex=d;return d}},count:{readOnly:true,getter:function(){return a.get("buttonNode")[0].options.length}}}),c=function(d,e){n.each(e,function(g){m.isHash(g)?v.each(g,
function(k,i){q(d,k,i)}):q(d,g)})},b=++jet.buttons.combo;a.on("optionsChange",function(d,e){var g=a.get("buttonNode")[0];g.options.length=0;c(g,e)});a.on("render",function(){var d=a.get("boundingBox"),e=a.get("buttonNode"),g=a.get("label");b=a.get("classPrefix")+a.get("className")+"-"+b;if(g){g=f("<label/>").html(g).appendTo(d);g[0].setAttribute("for",b)}e.attr("id",b).appendTo(d);e.on("change",function(){a.fire("change",e[0].options[e[0].selectedIndex])});c(a.get("buttonNode")[0],a.get("options"))})};
f.extend(s,o,{add:function(a,c){var b;if(c){b={};b[a]=c}else b=a;this.get("options").push(b);q(this.get("buttonNode")[0],a,c);return this},fill:function(a){var c=this;m.isArray(a)?n.each(a,function(b){c.add(b)}):v.each(a,c.add);return c},clear:function(){return this.set("options",[])},select:function(a){return this.set("selected",a)}});if(!jet.radioButtons)jet.radioButtons=1;var t=function(){t.superclass.constructor.apply(this,arguments);var a=++jet.radioButtons,c=[],b=this.addAttrs({options:{required:true},
